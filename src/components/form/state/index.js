...
const State = ({state, lengthSize, dispatch}) => {
  const [focused, setFocused] = useState(false);
  const translationY = new Animated.Value(state.length || focused ? 10 : 40);
  const color = new Animated.Value(focused ? 100 : 0);
  const handleState = e =>
    dispatch({
      type: FORM.CHANGE_STATE,
      payload: e,
    });
  const translationGo = () => {
    setFocused(true);
  ...
  const translationBack = () => {
    ...
    if (state.length === 0 && focused === false) {
      Animated.parallel([
        Animated.timing(translationY, {
          toValue: 40,
          duration: 500,
